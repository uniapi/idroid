...
## STDLIB Note
To bring **stdlib** onto the board we need to include `kotlin-stdlib.jar`.
Otherwise, such things like `List`s, `Map`s and others will crash the app without a trace.
Pass `ILIB=$KOTLIN_LIB/kotlib-stdlib.jar` only once after every clean `make clean` to optimize the build time.
