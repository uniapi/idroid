...
* **FrameLayout** *[API 1]* – A utility stack container designed to block out a region on the screen. It displays elements layered on top of each other, typically holding a single primary child or switching between fragments.
* **GridLayout** *[API 14]* – A powerful grid engine that arranges its children into a structural matrix of rows and columns. It supports row/column spanning concurrently and eliminates the nested-weight performance issues of legacy layouts.
...
* **Space** *[API 14]* – An ultra-lightweight, empty utility `View` designed specifically for generating blank spaces or margins inside layout architectures. It overrides `onDraw` to perform zero rendering operations, keeping measurement passes optimal.
* **FragmentContainerView** *[API 29]* – A specialized layout container extending `FrameLayout` engineered specifically to resolve historical z-indexing, window inset, and entry/exit animation bugs during dynamic `Fragment` transactions.
...
* **AbsoluteLayout** *[API 1]* **(Deprecated)** – A legacy container that allowed positioning elements using exact, hardcoded X/Y coordinates. _Avoid using this as it fails to scale across different screen resolutions_.
...
* **NestedScrollView** *[API 21]* – A modern layout container that behaves exactly like a vertical `ScrollView` but implements the native nested scrolling frameworks. It dynamically coordinates touch velocities and swipe gestures up and down the layout hierarchy (ideal for collapsing toolbars).
...
* **TextClock** *[API 17]* – A specialized extension of `TextView` that hooks directly into the system calendar ticks to output the exact current time and date in a customized format pattern string (e.g., `hh:mm:ss` a).
...
* **SurfaceView** *[API 1]* – Provides a dedicated drawing surface embedded directly inside the view hierarchy. It passes drawing calls to a secondary execution thread, bypassing the main UI thread to render fast animations, games, or video feeds.
* **TextureView** *[API 14]* – A modern graphics surface engine engineered to host real-time rendering layers (like hardware-accelerated video frames, camera viewfinders, or OpenGL contexts). Unlike `SurfaceView`, it behaves like a standard `View` node—meaning it can be smoothly animated, scaled, rotated, or blended with alpha opacity.
...
* **Toolbar** *[API 21]* – A highly customizable action-bar component that replaces the legacy application window `ActionBar`. Because it operates as a standard `ViewGroup` child node, it can be embedded anywhere within a layout tree to house navigation icons, titles, and custom menu items.
* **SearchView** *[API 11]* – A specialized entry widget built for executing search operations. It wraps an input field, clean buttons, and active voice hooks inside a uniform container designed to interface with the system search manager.

------------------------------
## 🗂 5. Heavy Data-Driven Adapter Views (Arrays)
Specialized layout components that extend `AdapterView`. They do not hold children directly in code; instead, they generate child rows dynamically using an external structural backing data provider called an `Adapter`.
...
* **AdapterViewFlipper** *[API 11]* – A specialized instance of an animated adapter view that automatically alternates between its data list rows at distinct intervals, operating effectively as an automated slideshow loop.
* **StackView** *[API 11]* – An advanced adapter-backed layout view that displays an array of items as a 3D deck of overlapping cards, allowing users to swipe individual cards away to expose the next node in the array.
...
↳ android.view.View [API 1]
  ...
  └── android.view.ViewGroup [API 1]
      ...
      │   └── androidx.fragment.app.FragmentContainerView [API 29]
      │
...
------------------------------
## ⚡ Pure Kotlin 1.3+ Multi-Era Layout Implementation
The following complete codebase demonstrates how to declare, set properties, configure layout relationships, and attach performance callbacks across multiple eras of the Android SDK inside a pure Kotlin environment.
```kotlin
package localhost.idroid.lessons
...
class PureCodeMasterActivity : Activity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)

        // 1. Initialization of the Scrollable Viewport (API 21 NestedScrollView)
        val mainScrollView = NestedScrollView(this).apply {
            layoutParams = NestedScrollView.LayoutParams(
                NestedScrollView.LayoutParams.MATCH_PARENT,
                NestedScrollView.LayoutParams.MATCH_PARENT
            )
            isFillViewport = true // Ensure full screen stretching matching XML properties
        }

        // 2. Initialization of the Core Structural Content Container (API 1 LinearLayout)
        val structuralContainer = LinearLayout(this).apply {
        ...
        // 3. Adding an Atomic Content View (API 17 TextClock)
        val textClockNode = TextClock(this).apply {
            layoutParams = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.WRAP_CONTENT,
                LinearLayout.LayoutParams.WRAP_CONTENT
            ).apply {
        ...
        // 4. Adding an Atomic Empty Spacer Utility View (API 14 Space)
        val itemSpacer = Space(this).apply {
            layoutParams = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                48 // Structural margin height allocation in pixels
            )
        }

        // 5. Adding an Interactive Multi-State Input Control (API 14 Switch)
        val statusToggle = Switch(this).apply {
            layoutParams = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT
            ).apply {
            ...
            text = "Enable Advanced Data Matrix Grid"
            isChecked = false
        }

        // 6. Building an Advanced Structural Matrix Grid Container (API 14 GridLayout)
        val dynamicGrid = GridLayout(this).apply {
            layoutParams = LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT
            )
            ...
            visibility = android.view.View.GONE // Initially hidden until toggle activation
        }

        // Populate the Matrix Grid rows entirely via pure code allocations
        for (row in 0 until 2) {
            for (col in 0 until 2) {
                val gridCellItem = TextView(this).apply {
                    text = "Cell [$row,$col]"
                    textSize = 14f
                    setPadding(24, 24, 24, 24)
                    
                    // Create specific structural assignment params for a standard cell placement
                ...
                dynamicGrid.addView(gridCellItem)
            }
        }

        // 7. Binding Event Callbacks to Handle State Intersect Flow
        statusToggle.setOnCheckedChangeListener { _, isChecked ->
            if (isChecked) {
                dynamicGrid.visibility = android.view.View.VISIBLE
                textClockNode.setTextColor(Color.parseColor("#00796B")) // Success Color Shift
            } else {
                dynamicGrid.visibility = android.view.View.GONE
                textClockNode.setTextColor(Color.DKGRAY)
            }
        }

        // 8. Re-assembling the Complete Nested Layout Node Tree Programmatically
        structuralContainer.addView(textClockNode)
        structuralContainer.addView(itemSpacer)
        structuralContainer.addView(statusToggle)
        structuralContainer.addView(dynamicGrid)
        
        mainScrollView.addView(structuralContainer)

        // Set the root display viewport view to the activity context window
