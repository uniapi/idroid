## Challenge 1: Pure-Code Profile Dashboard
## 🎯 Objective
...
4. **Bonus**
	* **The Logic Rule**: When the user toggles the `Switch` to **ON**, the status text inside the Header Section must immediately change its text to `"System Mode: ACTIVE"`
	and its text color to dark green. When toggled **OFF**, it must revert back to `"System Mode: Idle"` in a dark gray color.
